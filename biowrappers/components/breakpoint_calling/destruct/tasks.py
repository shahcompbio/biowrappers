...
def write_store(breakpoint_filename, breakpoint_library_filename, store_filename):
    breakpoint_table = pd.read_csv(
        breakpoint_filename, sep='\t',
        converters={'chromosome_1': str, 'chromosome_2': str},
    )

    breakpoint_library_table = pd.read_csv(
        breakpoint_library_filename, sep='\t',
    )
