        ...
        for key in sorted(in_files):
            ...
            if writer is None:
                ...
                reader.filters[FILTER_ID_BASE] = vcf.parser._Filter(
                    id=FILTER_ID_BASE, 
                    desc='Fraction of basecalls filtered at this site in either sample is at or above {0}'.format(max_filtered_basecall_frac)
                )
                
                reader.filters[FILTER_ID_SPANNING_DELETION] = vcf.parser._Filter(
                    id=FILTER_ID_SPANNING_DELETION,
                    desc='Fraction of reads crossing site with spanning deletions in either sample exceeeds {0}'.format(max_spanning_deletion_frac)
                )
                
                reader.filters[FILTER_ID_QSS] = vcf.parser._Filter(
                    id=FILTER_ID_QSS,
                    desc='Normal sample is not homozygous ref or ssnv Q-score < {0}, ie calls with NT!=ref or QSS_NT < {0}'.format(quality_lower_bound)
                )
        ...
        for key in sorted(vcf_files):
            ...
            if writer is None:
                ...
                reader.filters[FILTER_ID_INDEL_HPOL] = vcf.parser._Filter(
                    ...
                    desc='Indel overlaps an interrupted homopolymer longer than {0}x in the reference sequence'.format(max_int_hpol_length)
                )
                
                reader.filters[FILTER_ID_BASE] = vcf.parser._Filter(
                    id=FILTER_ID_BASE,
                    desc='Average fraction of filtered basecalls within 50 bases of the indel exceeds {0}'.format(max_window_filtered_basecall_frac)
                )
                
                reader.filters[FILTER_ID_QSI] = vcf.parser._Filter(
                    id=FILTER_ID_QSI,
                    desc='Normal sample is not homozygous ref or sindel Q-score < {0}, ie calls with NT!=ref or QSI_NT < {0}'.format(quality_lower_bound)
                )
                                
                writer = vcf.Writer(out_fh, reader)
            
            for record in reader:
                window_row = window.loc[(window['chrom'] == str(record.CHROM)) & (window['coord'] == record.POS)].iloc[0]
